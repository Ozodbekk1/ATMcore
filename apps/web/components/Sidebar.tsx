    ...
    <aside className="w-[260px] bg-[#071a14] border-r border-[#133c2e] flex flex-col justify-between hidden md:flex">
      <div>
        ...
        <nav className="px-4 space-y-1.5">
          {(role === 'admin' || role === 'superadmin') && <NavItem href="/" currentPath={pathname} icon={<LayoutGrid size={18} />} label="Overview" />}
          <NavItem href="/live-map" currentPath={pathname} icon={<Map size={18} />} label="Live Map" />
          {(role === 'admin' || role === 'superadmin') && (
            <>
              <NavItem href="/ai-predictions" currentPath={pathname} icon={<BrainCircuit size={18} />} label="AI Predictions" badge={aiBadge} />
      ...
      >
        <div className="bg-[#9de1b9] h-9 w-9 rounded-full flex items-center justify-center text-sm font-bold text-[#071a14] shadow-lg">
          {role === 'superadmin' ? 'SA' : role === 'admin' ? 'AM' : 'US'}
        </div>
        <div className="flex-1 overflow-hidden">
          <p className="text-sm font-medium text-[#e2f1ea] truncate">
            {role === 'superadmin' ? 'System Root' : role === 'admin' ? 'Alex Morgan' : 'Client Agent'}
          </p>
...
function NavItem({ href, currentPath, icon, label, badge }: { href: string, currentPath: string, icon: React.ReactNode, label: string, badge?: number }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`${active ? 'text-[#071a14]' : 'text-[#5d8573] group-hover:text-[#9de1b9]'} transition-colors`}>
          {icon}
        </div>
