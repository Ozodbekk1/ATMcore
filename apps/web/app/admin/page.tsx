...
import { fetchAtmList, fetchAdminUsers, fetchLogs, generateTestData, deleteAllAtms, type AtmData, type UserData, type LogData } from '../../lib/api';
import AtmImportModal from '../../components/AtmImportModal';
...
function TabButton({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-6 py-3 border-b-2 transition-all font-semibold ${active
...
function NetworkControl() {
  ...
  const handleClearAll = async () => {
    ...
    setClearResult(null);
    try {
      const res = await deleteAllAtms();
      setClearResult(`Cleared ${res.details?.atmsDeleted || 0} ATMs and ${res.details?.transactionsDeleted || 0} transactions`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[#0a241c] p-4 rounded-2xl border border-[#133c2e]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="flex items-center gap-2 bg-[#9de1b9] text-[#071a14] px-4 py-2 rounded-xl text-sm font-bold shadow-[0_0_15px_rgba(157,225,185,0.3)] hover:bg-[#b0ebd1] transition-all"
          >
            <Plus className="w-4 h-4" /> Add ATMs
          </button>
          {atms.length > 0 && (
            <button
              onClick={() => setShowClearConfirm(true)}
              className="flex items-center gap-2 bg-[#1f1115] text-[#fb7185] px-4 py-2 rounded-xl text-sm font-bold border border-rose-900/30 hover:bg-[#2a1520] transition-colors"
            >
      ...
      {showClearConfirm && (
        <div className="bg-[#1f1115] border border-rose-900/30 p-5 rounded-2xl animate-in slide-in-from-top duration-200">
          <div className="flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-[#fb7185] shrink-0 mt-0.5" />
            <div className="flex-1">
              <h4 className="text-[#fb7185] font-bold text-sm">Confirm Data Deletion</h4>
              <p className="text-xs text-[#fb7185]/70 mt-1">This will permanently delete all {atms.length} ATMs and their transaction history. This action cannot be undone.</p>
              <div className="flex gap-2 mt-4">
                <button
                  onClick={handleClearAll}
                  disabled={clearing}
                  className="flex items-center gap-2 bg-[#fb7185] text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#f75d75] transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold text-[#78a390] bg-[#0a241c] border border-[#133c2e] hover:bg-[#12382c] transition-colors"
                >
                  Cancel
                </button>
              </div>
      ...
      {/* Clear Result */}
      {clearResult && (
        <div className="p-3 bg-[#12382c] border border-[#1c5542] rounded-xl text-[#9de1b9] text-sm flex items-center gap-2 animate-in slide-in-from-top duration-200">
          <CheckCircle className="w-4 h-4" /> {clearResult}
        </div>
      )}
      ...
      {atms.length === 0 ? (
        <div className="bg-[#0a241c] border border-[#133c2e] p-8 rounded-2xl text-center space-y-4">
          <div className="flex justify-center">
            <div className="p-4 bg-[#061814] rounded-2xl">
              <Upload className="w-8 h-8 text-[#5d8573]" />
            </div>
          </div>
          <div>
            <p className="text-[#e2f1ea] font-semibold">No ATM nodes found</p>
            <p className="text-[#78a390] text-sm mt-1">Import ATM data using JSON or Google Sheets to get started.</p>
          </div>
          <button
            onClick={() => setShowImportModal(true)}
            className="inline-flex items-center gap-2 bg-[#9de1b9] text-[#071a14] px-5 py-2.5 rounded-xl text-sm font-bold shadow-[0_0_15px_rgba(157,225,185,0.3)] hover:bg-[#b0ebd1] transition-all"
          >
            <Plus className="w-4 h-4" /> Import ATM Data
          </button>
        </div>
      ) : (
              ...
              {users.map(user => (
                <tr key={user._id} className="hover:bg-[#071a14] transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold ${user.role === 'SUPERADMIN' ? 'bg-[#9de1b9] text-[#071a14]' :
