...
import { ShieldCheck, AlertTriangle, X, Target, Crosshair, ChevronRight, BrainCircuit, BarChart2, MapPin, Navigation, Loader2 } from 'lucide-react';
...
import { fetchAtmList, sendAnalyticsChat, type AtmData } from '../../lib/api';
...
// Group ATMs by region prefix (e.g., ATM-TAS → Tashkent Hub)
const regionNames: Record<string, string> = {
  'ATM-TAS': 'Tashkent Hub',
  'ATM-FER': 'Fergana Node',
  'ATM-SAM': 'Samarkand Core',
  'ATM-BUX': 'Bukhara Point',
  'ATM-NUK': 'Nukus Terminal',
  'ATM-TER': 'Termez Gateway',
  'ATM-NAV': 'Navoi Vault',
  'ATM-AND': 'Andijan Link',
};

function buildMarkersFromAtms(atms: AtmData[]): RegionMarker[] {
  // Group ATMs by region prefix
  const groups: Record<string, AtmData[]> = {};
  for (const atm of atms) {
    const prefix = atm.atmId.split('-').slice(0, 2).join('-');
    if (!groups[prefix]) groups[prefix] = [];
    groups[prefix].push(atm);
  }

  return Object.entries(groups).map(([prefix, groupAtms], idx) => {
    const totalCash = groupAtms.reduce((s, a) => s + a.currentCash, 0);
    const totalCapacity = groupAtms.reduce((s, a) => s + a.capacity, 0);
    const cashPercent = totalCapacity > 0 ? Math.round((totalCash / totalCapacity) * 100) : 0;
    const avgLat = groupAtms.reduce((s, a) => s + a.location.lat, 0) / groupAtms.length;
    const avgLng = groupAtms.reduce((s, a) => s + a.location.lng, 0) / groupAtms.length;
    const onlineCount = groupAtms.filter(a => a.status === 'ONLINE').length;
    const uptime = groupAtms.length > 0 ? ((onlineCount / groupAtms.length) * 100).toFixed(1) : '0';
    const totalTxns = groupAtms.length * 500 + Math.floor(Math.random() * 10000); // rough estimate

    const status = cashPercent <= 10 ? 'critical' : cashPercent <= 25 ? 'warning' : 'optimal';

    const mahallas = groupAtms.map(a => {
      const cp = a.capacity > 0 ? Math.round((a.currentCash / a.capacity) * 100) : 0;
      return {
        name: a.branch.split(' - ')[1] || a.branch,
        cash: `${cp}%`,
        status: cp <= 10 ? 'critical' : cp <= 25 ? 'warning' : 'optimal',
      };
    });

    return {
      id: idx + 1,
      name: regionNames[prefix] || prefix,
      ...
      details: {
        ...
        dailyTrend: cashPercent > 50 ? `+${(Math.random() * 5).toFixed(1)}%` : `-${(Math.random() * 8).toFixed(1)}%`,
        mahallas,
      },
    };
  });
...
// Fallback hardcoded markers (used when DB is empty)
const fallbackMarkers: RegionMarker[] = [
  { id: 1, name: "Tashkent Hub", coords: [41.2995, 69.2401], status: 'optimal', cash: "85%", details: { uptime: "99.9%", transactions: 15420, dailyTrend: "+4.2%", mahallas: [{ name: "Yunusobod 4-daha", cash: "92%", status: "optimal" }, { name: "Chilonzor 9-kv", cash: "15%", status: "warning" }, { name: "Mirzo Ulugbek", cash: "5%", status: "critical" }, { name: "Sirg'ali 7A", cash: "88%", status: "optimal" }] } },
  { id: 2, name: "Fergana Node", coords: [40.3864, 71.7864], status: 'optimal', cash: "60%", details: { uptime: "98.5%", transactions: 8200, dailyTrend: "-1.5%", mahallas: [{ name: "Marg'ilon Markaz", cash: "60%", status: "optimal" }, { name: "Qo'qon Avtoshow", cash: "45%", status: "optimal" }, { name: "Farg'ona City", cash: "81%", status: "optimal" }] } },
  { id: 3, name: "Samarkand Core", coords: [39.6270, 66.9749], status: 'warning', cash: "25%", details: { uptime: "97.2%", transactions: 12050, dailyTrend: "+12.1%", mahallas: [{ name: "Registon Maydoni", cash: "20%", status: "warning" }, { name: "Siyob Bozori", cash: "10%", status: "critical" }, { name: "So'g'diyona Makro", cash: "55%", status: "optimal" }] } },
  { id: 4, name: "Bukhara Point", coords: [39.7681, 64.4556], status: 'optimal', cash: "70%", details: { uptime: "99.1%", transactions: 6400, dailyTrend: "+2.0%", mahallas: [{ name: "Labi Hovuz", cash: "65%", status: "optimal" }, { name: "Buxoro Vokzal", cash: "78%", status: "optimal" }] } },
  { id: 5, name: "Nukus Terminal", coords: [42.4608, 59.6105], status: 'critical', cash: "5%", details: { uptime: "92.4%", transactions: 2100, dailyTrend: "-8.4%", mahallas: [{ name: "Nukus Markaziy Box", cash: "3%", status: "critical" }, { name: "Amudaryo Mahalla", cash: "7%", status: "critical" }] } },
  { id: 6, name: "Termez Gateway", coords: [37.2242, 67.2783], status: 'optimal', cash: "90%", details: { uptime: "99.8%", transactions: 3500, dailyTrend: "+0.5%", mahallas: [{ name: "Termez City Mall", cash: "88%", status: "optimal" }, { name: "Aeroport", cash: "92%", status: "optimal" }] } },
  { id: 7, name: "Navoi Vault", coords: [40.0844, 65.3792], status: 'optimal', cash: "65%", details: { uptime: "99.5%", transactions: 4400, dailyTrend: "+1.9%", mahallas: [{ name: "Bozor Hududi", cash: "50%", status: "optimal" }, { name: "Navoi AZS", cash: "85%", status: "optimal" }] } },
  { id: 8, name: "Andijan Link", coords: [40.8154, 72.2837], status: 'optimal', cash: "80%", details: { uptime: "98.9%", transactions: 9100, dailyTrend: "+4.7%", mahallas: [{ name: "Jahon Bozori", cash: "75%", status: "optimal" }, { name: "Eski Shahar", cash: "82%", status: "optimal" }] } },
];
...
function MapContent() {
  ...
  const [markers, setMarkers] = useState<RegionMarker[]>(fallbackMarkers);
    ...
    (async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const built = buildMarkersFromAtms(res.data);
          if (built.length > 0) {
            setMarkers(built);
          }
        }
  ...
  const handleSendMessage = async (overrideText?: string | React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
      const contextMessage = selectedRegion 
        ? `[Context: ATM Region "${selectedRegion.name}", Cash Level: ${selectedRegion.cash}, Status: ${selectedRegion.status}, Uptime: ${selectedRegion.details.uptime}]\n\nUser Question: ${textToSend}`
        : textToSend;
      
      const res = await sendAnalyticsChat(contextMessage);
      setChatHistory(prev => [...prev, { role: 'ai', text: res.reply || 'No response received.' }]);
    } catch (err: any) {
      setChatHistory(prev => [...prev, { 
        role: 'ai', 
        text: `Error: ${err.message || 'Failed to get AI response. Please try again.'}`
      }]);
    } finally {
    ...
    <div className="relative w-full h-[calc(100vh-80px)] overflow-hidden bg-[#03110d]">
      ...
      <div className={`absolute top-0 left-0 w-full z-[5] p-4 sm:p-6 pointer-events-none flex flex-col sm:flex-row justify-between items-start gap-4 transition-all duration-500 ${selectedRegion ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="pointer-events-auto bg-[#071a14]/90 backdrop-blur-md p-4 rounded-2xl border border-[#133c2e] shadow-2xl flex flex-col gap-1">
          ...
          <p className="text-xs text-[#78a390]">Full-screen geographic zoom active • {markers.length} regions</p>
        </div>
      ...
      <div className={`absolute top-[120px] left-6 bg-[#04120e]/80 backdrop-blur-xl border-l-2 border-l-[#9de1b9] border border-[#133c2e]/50 p-6 rounded-r-2xl shadow-2xl w-64 pointer-events-auto z-10 transition-all duration-500 hidden sm:block ${selectedRegion ? '-translate-x-[150%] opacity-0' : 'translate-x-0 opacity-100'}`}>
         ...
         <div className="space-y-4 font-mono">
            ...
            <div className="flex justify-between items-center border-b border-[#133c2e]/50 pb-2">
               ...
               <span className="text-[#e2f1ea] font-bold text-xs">{markers.length} / {markers.length}</span>
            </div>
            <div className="flex justify-between items-center">
               <span className="text-[#5d8573] text-[10px] uppercase tracking-wider">Critical Sync</span>
               <span className={`font-bold text-xs flex items-center gap-1.5 ${activeMarkers.length > 0 ? 'text-[#fb7185]' : 'text-[#9de1b9]'}`}>
                 {activeMarkers.length > 0 ? <><AlertTriangle className="w-3 h-3 animate-pulse" /> {activeMarkers.length} NODE{activeMarkers.length > 1 ? 'S' : ''}</> : 'ALL CLEAR'}
               </span>
            </div>
         </div>
      </div>
          ...
          <div className="h-full flex flex-col">
            ...
            {showAIChatPanel ? (
                  ...
                  <div className="bg-[#0a241c] border border-[#1c5542] p-5 rounded-2xl rounded-tl-sm self-start inline-block w-[90%] text-[#e2f1ea] shadow-lg text-[13px] font-mono leading-relaxed mt-2">
                   ...
                   {chatHistory.map((msg, i) => (
                     msg.role === 'user' ? (
                       ...
                       <div key={i} className="bg-[#0a241c] border border-[#1c5542] p-5 rounded-2xl rounded-tl-sm self-start inline-block w-[90%] text-[#e2f1ea] shadow-lg text-[13px] font-mono leading-relaxed relative mt-4 float-left clear-both break-words whitespace-pre-wrap">
                          <div className="absolute -left-2 top-0 w-2 h-2 rounded-full bg-[#9de1b9] shadow-[0_0_8px_#9de1b9]"></div>
                   ...
                   {isSendingChat && (
                     <div className="float-left clear-both mt-4 flex items-center gap-2 text-[#9de1b9] text-xs font-mono">
                ...
                <div className="p-5 border-t border-[#133c2e] bg-[#04120e]">
                   <div className="relative">
                     <input 
                       ...
                       className="w-full bg-[#03110d] border border-[#133c2e] p-4 rounded-2xl text-[#e2f1ea] pr-14 focus:outline-none focus:border-[#9de1b9] focus:ring-1 focus:ring-[#9de1b9] placeholder-[#5d8573] font-sans text-[13px] transition-all shadow-inner disabled:opacity-50" 
                       placeholder="Ask Nexus AI about this node..." 
                     />
                     <button 
                       onClick={() => handleSendMessage()}
                       disabled={isSendingChat}
                       className="absolute right-2 top-2 p-2 bg-[#12382c] border border-[#1c5542] rounded-xl text-[#9de1b9] hover:bg-[#1a4a3a] transition-colors shadow-lg disabled:opacity-50">
                        {isSendingChat ? <Loader2 size={18} className="animate-spin" /> : <ChevronRight size={18} />}
                     </button>
                   </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-[#133c2e] pb-12">
                <h4 className="text-[10px] font-bold text-[#5d8573] uppercase tracking-[0.2em] mb-4">Micro-Zones (Terminals)</h4>
                <div className="space-y-3">
                  {selectedRegion.details.mahallas.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                         ...
                         <div className="flex flex-col items-end gap-1">
                           <span className={`font-bold text-[10px] px-2 py-0.5 rounded-md border transition-colors ${isOpt ? 'text-[#9de1b9] bg-[#12382c] border-[#1c5542]' : isWarn ? 'text-amber-400 bg-[#3d3121] border-amber-900/50' : 'text-[#fb7185] bg-[#0a241c] border-[#1c5542]'}`}>{atm.distance.toFixed(2)} km</span>
                         </div>
                      </div>
                    );
                  })}
